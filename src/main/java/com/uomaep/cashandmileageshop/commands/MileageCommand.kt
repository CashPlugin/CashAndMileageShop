...
class MileageCommand : CommandExecutor, TabCompleter {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>?): Boolean {
        ...
        if (args?.size == 0) {
            ...
            if (mileage == -1) {
                UserMessage.sendFailMessage(player, "마일리지를 불러오는 도중 오류가 발생했습니다.")
                return false
            }
            UserMessage.sendSuccessMessage(player, "마일리지: §a${mileage}§f원")
            return true
        }

        if (!player.isOp) {
            UserMessage.sendFailMessage(player, "/마일리지 §7- 마일리지를 확인합니다.")
            return false
        }
        ...
        if (args?.size == 2) {
            when (args[0]) {
                "보기" -> {
                    ...
                    UserMessage.sendSuccessMessage(player, "§e${nickname}§f님의 마일리지: §a${mileage}§f원")
                    return true
                }
            ...
            try {
                ...
                when (args[0]) {
                    "설정" -> {
                        ...
                        UserMessage.sendSuccessMessage(player, "§e${nickname}§f님의 마일리지를 §a${balance}§f원으로 설정했습니다.")
                        return true
                    }
                    ...
                    "차감" -> {
                        ...
                        UserMessage.sendSuccessMessage(player, "§e${nickname}§f님의 마일리지에서 §c${balance}§f원을 차감했습니다.")
                        return true
                    }
